...
export function detectByLogSx(receipt: ethers.TransactionReceipt): Promise<boolean> {
  ...
  return new Promise(resolve => {
    const matchFound = receipt.logs.some((log) => flashLoanEventSignatures.some((signatureHash) => log.topics[0] === signatureHash))
    resolve(matchFound)
  })
}
